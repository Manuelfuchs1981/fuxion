...
export default function KontaktModal({ kontakt, onClose, onSaved }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const kontaktData = {
        typ, firma: firma || null, branche: branche || null,
        mwst_nr: mwstNr || null, website: website || null,
        iban: iban || null, adresse: adresse || null,
        plz: plz || null, ort: ort || null, land,
        notizen: notizen || null, kundennummer,
      ...
      const personenInsert = personen.map((p, i) => ({
        kontakt_id: kontaktId!, user_id: user.id,
        personennummer: getPersonNr(i),
        anrede: p.anrede || null, vorname: p.vorname || null,
        nachname: p.nachname, funktion: p.funktion || null,
        email: p.email || null, telefon: p.telefon || null,
        mobile: p.mobile || null, ist_hauptperson: i === 0,
      }))
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setTyp('firma')} className={`flex items-center justify-center gap-2 p-3 rounded-xl border-2 font-semibold text-sm transition-all ${typ === 'firma' ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}>
              <Building2 className="w-4 h-4" /> Firma / Organisation
            </button>
            <button onClick={() => setTyp('privat')} className={`flex items-center justify-center gap-2 p-3 rounded-xl border-2 font-semibold text-sm transition-all ${typ === 'privat' ? 'border-blue-500 bg-blue-50 text-blue-600' : 'border-gray-200 text-gray-500 hover:border-gray-300'}`}>
          ...
          {typ === 'firma' && (
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={firma} onChange={e => setFirma(e.target.value)} placeholder="z.B. Morf aG" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={branche} onChange={e => setBranche(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                  ...
                  <input value={mwstNr} onChange={e => setMwstNr(e.target.value)} placeholder="CHE-123.456.789" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={website} onChange={e => setWebsite(e.target.value)} placeholder="www.beispiel.ch" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  ...
                  <input value={iban} onChange={e => setIban(e.target.value)} placeholder="CH56 0483 5012 3456 7800 9" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <input value={adresse} onChange={e => setAdresse(e.target.value)} placeholder="Strasse und Hausnummer" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <div className="grid grid-cols-3 gap-3">
              <input value={plz} onChange={e => setPlz(e.target.value)} placeholder="PLZ" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <input value={ort} onChange={e => setOrt(e.target.value)} placeholder="Ort" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              <select value={land} onChange={e => setLand(e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between pb-1 border-b border-gray-100">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">{typ === 'firma' ? 'Kontaktpersonen' : 'Person'}</p>
              {typ === 'firma' && <span className="text-xs text-gray-400">Jede Person erhält eine Personennummer (KP-)</span>}
            </div>
            <div className="space-y-2">
              {personen.map((p, i) => (
                <div key={i} className="border border-gray-200 rounded-xl overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 bg-gray-50 cursor-pointer hover:bg-gray-100" onClick={() => updatePerson(i, 'expanded', !p.expanded)}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-mono bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-semibold">{getPersonNr(i)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      {i > 0 && (
                        <button onClick={e => { e.stopPropagation(); removePerson(i) }} className="flex items-center gap-1 text-xs text-gray-400 hover:text-red-500 px-2 py-1 rounded hover:bg-red-50">
                    ...
                    <div className="p-4 space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select value={p.anrede} onChange={e => updatePerson(i, 'anrede', e.target.value)} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div>
                          ...
                          <input value={p.funktion} onChange={e => updatePerson(i, 'funktion', e.target.value)} placeholder="z.B. Geschäftsführer" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-semibold text-gray-700 mb-1">Vorname</label>
                          <input value={p.vorname} onChange={e => updatePerson(i, 'vorname', e.target.value)} placeholder="Vorname" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-700 mb-1">Nachname *</label>
                          <input value={p.nachname} onChange={e => updatePerson(i, 'nachname', e.target.value)} placeholder="Nachname" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-semibold text-gray-700 mb-1">E-Mail</label>
                          <input type="email" value={p.email} onChange={e => updatePerson(i, 'email', e.target.value)} placeholder="email@beispiel.ch" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                          ...
                          <input type="tel" value={p.mobile} onChange={e => updatePerson(i, 'mobile', e.target.value)} placeholder="+41 79 123 45 67" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input type="tel" value={p.telefon} onChange={e => updatePerson(i, 'telefon', e.target.value)} placeholder="+41 44 123 45 67" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-gray-700 mb-1">Personennummer</label>
                          <input value={getPersonNr(i) || ''} readOnly className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-500 font-mono cursor-default" />
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {typ === 'firma' && (
              <button onClick={addPerson} className="flex items-center justify-center gap-2 w-full py-2.5 border-2 border-dashed border-gray-200 rounded-xl text-sm text-gray-500 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-all font-medium">
          ...
          <div className="space-y-2">
            ...
            <textarea value={notizen} onChange={e => setNotizen(e.target.value)} rows={2} placeholder="Interne Notizen zum Kunden…" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 sticky bottom-0 bg-white rounded-b-2xl">
          ...
          <button onClick={handleSave} disabled={saving} className="px-5 py-2 text-sm font-semibold bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
